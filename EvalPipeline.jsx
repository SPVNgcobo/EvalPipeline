...
function ScoreBar({ value, max = 5 }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontSize: 11, color, fontFamily: "monospace", minWidth: 20 }}>{value}</span>
    </div>
  );
}
...
export default function EvalPipeline() {
  ...
  const [evals, setEvals] = useState(SAMPLE_EVALS);
  ...
  const flagCount = evals.filter(e => e.verdict === "flag").length;
  const failCount = evals.filter(e => e.verdict === "fail").length;
  const avgScore = evals.reduce((s, e) => s + weightedScore(e.scores), 0) / evals.length;
  ...
  const issueFreq = {};
  ISSUE_TAGS.forEach(t => { issueFreq[t] = evals.filter(e => e.issues.includes(t)).length; });
  ...
  function handleSubmitEval() {
    ...
    const entry = { ...newEval, id, timestamp: new Date().toISOString(), evaluator: "S.Ngcobo" };
    setEvals(prev => [entry, ...prev]);
    setSubmitted(true);
    setTimeout(() => {
      setSubmitted(false);
      setNewEval({ model: MODELS[0], prompt: "", response: "", scores: { accuracy: 3, relevance: 3, clarity: 3, safety: 3, instruction: 3 }, issues: [], verdict: "pass", notes: "" });
      setTab("log");
    }, 1200);
  }
  ...
  const compareEvals = compareIds.map(id => evals.find(e => e.id === id)).filter(Boolean);
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginLeft: 2, fontFamily: "monospace" }}>v2.0</span>
          </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        ...
        {tab === "dashboard" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              ...
              <StatCard label="Pass Rate" value={`${Math.round((passCount / evals.length) * 100)}%`} sub={`${passCount} passed`} accent="#00e5a0" />
                ...
                {ISSUE_TAGS.filter(t => issueFreq[t] > 0).map(t => (
                  <div key={t} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1, height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
                      <div style={{ width: `${(issueFreq[t] / evals.length) * 100}%`, height: "100%", background: "#ff4d6d", borderRadius: 2 }} />
                    </div>
        ...
        {tab === "evaluate" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 720 }}>
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>New Evaluation</h1>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <label style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>Model</label>
                <select value={newEval.model} onChange={e => setNewEval(p => ({ ...p, model: e.target.value }))}>
                  {MODELS.map(m => <option key={m}>{m}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>Verdict</label>
                <select value={newEval.verdict} onChange={e => setNewEval(p => ({ ...p, verdict: e.target.value }))}>
            ...
            <div>
              <label style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>System / User Prompt</label>
              <textarea rows={3} placeholder="Enter the prompt given to the AI..." value={newEval.prompt} onChange={e => setNewEval(p => ({ ...p, prompt: e.target.value }))} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>AI Response</label>
              <textarea rows={5} placeholder="Paste the AI response here..." value={newEval.response} onChange={e => setNewEval(p => ({ ...p, response: e.target.value }))} />
            </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 20 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 16, fontFamily: "monospace", color: scoreColor(weightedScore(newEval.scores)), fontWeight: 700 }}>
              ...
              {CRITERIA.map(c => (
                <div key={c.id} style={{ marginBottom: 16 }}>
                  ...
                  <input type="range" min={1} max={5} step={1} value={newEval.scores[c.id]}
                    onChange={e => setNewEval(p => ({ ...p, scores: { ...p.scores, [c.id]: Number(e.target.value) } }))}
                    style={{ width: "100%", accentColor: scoreColor(newEval.scores[c.id]) }} />
                </div>
              ))}
            </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {ISSUE_TAGS.map(t => {
                  ...
                  return (
                    <span key={t} className="issue-tag" onClick={() => setNewEval(p => ({
            ...
            <div>
              <label style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>Evaluator Notes</label>
              <textarea rows={2} placeholder="Optional notes, context, or observations..." value={newEval.notes || ""} onChange={e => setNewEval(p => ({ ...p, notes: e.target.value }))} />
            </div>

            <button className="action-btn" onClick={handleSubmitEval}
              disabled={!newEval.prompt || !newEval.response || submitted}
              style={{
                ...
                cursor: newEval.prompt && newEval.response ? "pointer" : "not-allowed",
                opacity: !newEval.prompt || !newEval.response ? 0.5 : 1
              }}>
              {submitted ? "✓ Logged Successfully" : "Submit Evaluation"}
            </button>
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              ...
              {compareIds.length === 2 && (
                <button className="action-btn" onClick={() => setTab("compare")} style={{
                  ...
                  color: "#00e5a0", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <input type="text" placeholder="Search prompts & responses..." value={searchQ}
                onChange={e => setSearchQ(e.target.value)} style={{ flex: 1, minWidth: 200 }} />
              <select value={filterVerdict} onChange={e => setFilterVerdict(e.target.value)} style={{ width: 150 }}>
                <option value="all">All Verdicts</option>
                <option value="pass">Pass</option>
                <option value="flag">Flag</option>
                <option value="fail">Fail</option>
              </select>
              <select value={filterModel} onChange={e => setFilterModel(e.target.value)} style={{ width: 180 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "20px 80px 1fr 130px 60px 70px 80px", gap: 0, padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                {["", "ID", "Prompt", "Model", "Score", "Verdict", "Compare"].map((h, i) => (
              ...
              {filteredEvals.map(e => {
                ...
                return (
                  <div key={e.id} className="eval-row" style={{ display: "grid", gridTemplateColumns: "20px 80px 1fr 130px 60px 70px 80px", gap: 0, padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.04)", alignItems: "center", cursor: "pointer" }}>
                    ...
                    <span onClick={() => setSelected(e)} style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingRight: 12 }}>{e.prompt}</span>
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <div>
                ...
                <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{selected.model}</h2>
              </div>
